...
## 파일 구성
- `task-tracker/index.html` : 메인 사용자 인터페이스. 체크리스트, 탭, 드래그 순서 조절, 알람 UI, PWA 설치 버튼을 포함합니다.
- `task-tracker/alarm-worker.js` : 웹 워커로 알람 시점을 주기적으로 확인하고 메인 스레드에 알림 이벤트를 보냅니다.
- `task-tracker/manifest.json` : PWA 설치를 위한 앱 메타데이터를 정의합니다.
- `task-tracker/sw.js` : 서비스 워커 초기 등록 스크립트입니다.
...
## 사용 정보
- `task-tracker/index.html`을 브라우저에서 직접 열어 기본 기능을 테스트할 수 있습니다.
